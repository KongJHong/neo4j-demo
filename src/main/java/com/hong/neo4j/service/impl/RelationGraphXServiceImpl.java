...
import com.hong.neo4j.repository.GraphNodeRepository;
...
public class RelationGraphXServiceImpl implements RelationGraphXService {
	...
	public RelationGraphXServiceImpl(GraphNodeRepository graphNodeRepository) {
	...
	public List<GraphNode> findAllNodes() {
		Iterable<GraphNode> pp = graphNodeRepository.findAll();
	...
	public List<GraphNode> addTest() {
		List<GraphNode> list = new ArrayList<>(30);
		// 添加人
		for (int i = 0;i < 30;i++) {
			GraphNode p = new GraphNode();
			p.setDate(new Date());
			p.setTitle(randomName());
			p.setType(NodeTypeEnum.PERSON);
			list.add(p);
		}
		...
		for (GraphNode p : list) {
			p.addPointing(list.get(random.nextInt(list.size())));
		}

		graphNodeRepository.saveAll(list);
		return null;
	}

	@Override
	public List<GraphNode> addTest2() {
		String[] movies = {"电影1","电影2","电影3","电影4"};
		Random random = new Random();
		List<GraphNode> persons = new ArrayList<>();
		List<GraphNode> mm = new ArrayList<>();
		graphNodeRepository.findAll().forEach(persons::add);

		for(int i = 0; i < 10;i++) {
			GraphNode node = new GraphNode();
			node.setTitle(movies[random.nextInt(movies.length)]);
			node.setDate(new Date());
			node.setType(NodeTypeEnum.MOVIE);
			mm.add(node);
			persons.get(random.nextInt(persons.size())).addPointing(node);
		}

		graphNodeRepository.saveAll(mm);
		graphNodeRepository.saveAll(persons);
		return null;
	}

	@Override
	public List<GraphNode> findAllOutNodeByTitle(String title) {
		if(StringUtils.isBlank(title)) return null;
		return graphNodeRepository.findAllOutNodeByTitle(title);
	}

	@Override
	public List<GraphNode> findAllOutNodeByTitleAndType(String title, NodeTypeEnum type) {
		if (StringUtils.isBlank(title) || type == null)return null;
		return graphNodeRepository.findAllOutNodeByTitleAndType(title, type);
	}



	@Override
	public List<GraphNode> findAllPersonByName(String title) {
		if (StringUtils.isBlank(title))return null;
		return graphNodeRepository.findAllInOutNodeByTitle(title);
	}
	...
	private static String randomName() {
		...
		Random random = new Random();
		return firsname[random.nextInt(firsname.length)] + namelist[random.nextInt(namelist.length)];
	}
}
